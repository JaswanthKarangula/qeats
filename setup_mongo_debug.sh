...
QEATS_SHARED_RESOURCES="${HOME}/Projects/Java/qeats_shared_resources"
if [ ! -d $QEATS_SHARED_RESOURCES ]
then
    git clone https://github.com/JaswanthKarangula/QEatsData $QEATS_SHARED_RESOURCES
else
    cd $QEATS_SHARED_RESOURCES
    git pull
fi

if ps -ef | grep mongod | grep -v grep | wc -l | tr -d ' '; then
